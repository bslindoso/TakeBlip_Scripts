...
function getMenuForPlatform(menuFormat) {
    ...
    switch (platform) {
        ...
        case 'messenger':
            ...
            break;
        case 'sunco':
            definePlatform = defineTypeSunco(menuScope, menu, boldValue);
            break;
        default:
            definePlatform = defineTypeBlipChat(menuScope, menu, boldValue);
            break;
...
function returnFormatMenu(menuScope) {
    ...
    if (menuScope.suncoQuickReply == true && platform == 'sunco') {
        return 'suncoQuickReply';
    }
    if (menuScope.defaultText == true && (platform == 'whatsapp' || platform == 'blipchat' || platform == 'sunco')) {
        return 'defaultText';
    }
    if (menuScope.defaultTextInverted == true && (platform == 'whatsapp' || platform == 'blipchat' || platform == 'sunco')) {
...
function defineTypeSunco(menuScope, Menu, boldValue) {
    let returnMenu = '';
    let typeMenu = returnFormatMenu(menuScope);

    switch (typeMenu) {
        case 'suncoQuickReply':
            returnMenu = getSuncoQuickReply(Menu);
            break;
        default:
            returnMenu = getTextMenu(Menu, boldValue);
            break;
...
function getSuncoQuickReply(Menu) {
    const menuOptions = [];
    for (let i = 0; i < Menu.options.length; i++) {
        menuOptions.push({
            "type": 'reply',
            "text": Menu.options[i],
            "payload": Menu.values[i]
        });
    }
    return {
        "content": {
